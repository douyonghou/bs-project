...
public class StockQueryModel {
	 ...
	 * @throws Exception
	 */
	public List<StockInfoBean> getOwnStock(String user) throws Exception {
		String sql = "select users,user_id,stock_id,create_time,stock_price,stock_number from stock_server where users = ?";
		Connection con = db.getConnection();
		PreparedStatement pst = con.prepareStatement(sql);
		...
		while (rs.next()) {
			...
			String stock_number = rs.getString("stock_number");
			stockList.add(new StockInfoBean(users, stock_id, create_time, stock_price,stock_number));
		}
		db.commit();
		db.close();
		return stockList;
