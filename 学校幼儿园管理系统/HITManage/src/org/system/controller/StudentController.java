...
public class StudentController {
    ...
    public String addStuent(@RequestParam String id,@RequestParam String password,
    		...
    		@RequestParam String phonenum,Map<String,List<Student>> map) throws UnsupportedEncodingException {
		//???????????????????
    	Student student = ss.queryById(id);	
    	if(student==null){   //学号已存在就不插了
    		int yearnum = Integer.parseInt(year);
			String newPassword = Md5.md5(password);
        	ss.addStudent(id, newPassword, name, yearnum, phonenum);
...
{    
	  ...
	  //  System.out.println(nameStr);
		...
		//	System.out.println(name);
			...
			System.out.println("û��osi-8859-1");
			e1.printStackTrace();
		}
